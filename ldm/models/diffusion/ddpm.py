...
class LatentDiffusionLLIE(DDPM):
    """main class"""
    def __init__(self,
                 first_stage_config,
                 cond_stage_config,
        ...
        ## Custom Definition
        self.dwt_enc = dwt_encode.DWT().to(self.device)
    ...
    def get_input(self, batch, k=None, return_first_stage_outputs=False, force_c_encode=False,
        ...
        self.short_img = batch['short_img'].cuda()
        self.short_img = self.short_img.to(memory_format=torch.contiguous_format).float()

        self.long_img = batch['long_img'].cuda()
        self.long_img = self.long_img.to(memory_format=torch.contiguous_format).float()
        ...
        dwt_feats = dwt_encode.multi_dwt_enc(x, self.dwt_enc)
        z = dwt_feats['x_LL_x8']
        ...
        if return_first_stage_outputs:
            ...
            out.extend([x, self.long_img, xrec])
        if return_original_cond:
            out.append(xc)
    ...
    # Custom Function
    def forward_wenqiang(self, x_noisy, cond, struct_cond, noise_scheduler):

        # t_ori = torch.tensor([self.ori_timesteps[index_i] for index_i in index])
        # t_ori = t_ori.long().to(x.device)

        for t in tqdm(noise_scheduler.timesteps):
            t_ori = torch.tensor([t]*x_noisy.shape[0]).long().to(x_noisy.device)
            struc_c = self.structcond_stage_model(struct_cond, t_ori)
            noise_pred = self.apply_model(x_noisy, t_ori, cond, struc_c)
            x_noisy = noise_scheduler.step(noise_pred.cpu(), t, x_noisy.cpu()).prev_sample.to(x_noisy.device)
        return x_noisy
    ...
    def log_images(self, batch, N=8, n_row=4, sample=True, ddim_steps=200, ddim_eta=1., return_keys=None,
        ...
        if self.test_gt:
            ...
            log["inputs"] = x
            with torch.no_grad():
                log["gts"] = self.decode_first_stage(self.encode_first_stage(gt).sample()*self.scale_factor)
                log["recon_inputs"] = self.decode_first_stage(z)
