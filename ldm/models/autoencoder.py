...
class AutoencoderKLResi(pl.LightningModule):
    ...
    def encode_dwt(self, x):
        input_dwt_feats = dwt_encode.multi_dwt_enc(x, self.dwt_enc)
        return input_dwt_feats
    ...
    def decode_dwt(self, x, input_dwt_feats):
        with torch.no_grad():
            dec = self.decode(x, input_dwt_feats)
        return dec

    def forward(self, input, input_dwt_feats, gt_latents):
        with torch.no_grad():
            posterior, _ = self.encode(input)
        dec = self.decode(gt_latents, input_dwt_feats)
        return dec, posterior
    ...
    def get_input(self, batch):
        short = batch['short_img']
        long = batch['long_img']

        input = short.to(memory_format=torch.contiguous_format).float()
        gt = long.to(memory_format=torch.contiguous_format).float()
        
        with torch.no_grad():
            gt_posterior, _ = self.encode(gt)
            gt_latent = gt_posterior.sample() / 0.18215

        input_dwt_feats = dwt_encode.multi_dwt_enc(input, self.dwt_enc)

        return input, gt, input_dwt_feats, gt_latent

    def training_step(self, batch, batch_idx, optimizer_idx=None):
        inputs, gts, input_dwt_feats, gt_latents = self.get_input(batch)
        reconstructions, posterior = self(gts, input_dwt_feats, gt_latents)
    ...
    def log_images(self, batch, only_inputs=False, **kwargs):
        ...
        log["gts"] = gts
        with torch.no_grad():
            reconstructions, _ = self(gts, input_dwt_feats, gt_latents)
        log["recons"] = reconstructions
        return log
